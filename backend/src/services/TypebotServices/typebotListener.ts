...
const typebotListener = async ({
    ...
    // console.log("type", ticket.companyId)
    const queue = ticket.queue;//await ShowQueueService(ticket.queueId, ticket.companyId)

    const { typebotUrl: url,
        typebotExpires,
    ...
    async function createSession(msg, number) {
        try {
    ...
    try {
        ...
        if (String(body) !== typebotKeywordFinish && String(body) !== typebotKeywordRestart) {
            let requestContinue
            let messages
            let input
        ...
        if (String(body) === typebotKeywordRestart) {
            await ticket.update({
                chatbot: true,
                typebotSessionId: null
        ...
        if (String(body) === typebotKeywordFinish) {
