...
const verifyQueue = async (
  ...
  if (queues.length === 1) {
    ...
    if (firstQueue.typebotUrl?.length > 0 && firstQueue.typebotStatus) {

      await typebotListener({ wbot, msg, ticket });
  ...
  if (choosenQueue) {
    ...
    if (choosenQueue.options.length === 0) {
      ...
      if (choosenQueue.typebotUrl?.length > 0 && choosenQueue.typebotStatus) {
      ...
      if (!isNil(choosenQueue.greetingMessage) && choosenQueue.greetingMessage !== "") {
        const sentMessage = await wbot.sendMessage(
          `${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, {
          text: body,
        }
